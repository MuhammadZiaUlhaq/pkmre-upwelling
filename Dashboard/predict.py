...
def load_model():
    model_path = 'Dashboard/data/model_fix.pkl'  # Replace with the correct path
...
def process_data():
    ...
    if st.button("Predict Upwelling"):
        try:
            ...
            input_array = np.array(input_features)  # Convert input to numpy array
            st.write(f"Input Array: {input_array}")  # Display input array for debugging
            hasil_prediksi = model.predict(input_array)
